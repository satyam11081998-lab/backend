...
# Copied services/interview_engine.py on 2026-09-23 for the role/company-aware Prep Copilot (v2).
...
from services.copilot.engine.prompts_interview import (
    build_interviewer_messages,
    CONVERSATION_SCORING_SYSTEM_PROMPT,
    build_conversation_scoring_user_prompt,
)
from services.copilot.engine.prompts_interview_v2 import build_adaptive_interviewer_messages
from services.copilot.engine.session_signals import compute_signals, needs_contextual_assessment, build_signal_block
from services.copilot.engine.interviewer_mode import get_modality_instruction, ALLOW_QUESTIONS, build_mode_block
from services.copilot.engine.interviewer_decision import (
    StreamTagStripper, parse_control_tag, sanitize_reply, enforce_mode, 
    assess_context_with_llm, evaluate_intervention_gate
)
from services.copilot.engine.learning_model import evaluate_intervention_outcome, build_learning_block
...
def _score_case_conversation(
    ...
    from services.copilot.engine.scorer import _enforce_case, _rejection_case, _is_hard_reject
...
def score_conversation(
    ...
    if is_guesstimate:
        from services.copilot.engine.scorer import score_guesstimate_answer, AIScoringError
